...
final savingsSheetProvider = FutureProvider.family<Map<String, dynamic>, String>((ref, cacheKey) async {
  ...
  try {
    ...
    await box.put('savings_sheet_$cacheKey', data);
    return data;
  } catch (e, st) {
    debugPrint('savingsSheetProvider error: $e\n$st');
    final box = Hive.box(HiveBoxes.savingsAccounts);
    final cached = box.get('savings_sheet_$cacheKey');
    if (cached != null) return Map<String, dynamic>.from(cached as Map);
    rethrow;
  }
});

class SavingsCollectionScreen extends ConsumerStatefulWidget {
  final String frequency;
  const SavingsCollectionScreen({super.key, required this.frequency});

  @override
  ConsumerState<SavingsCollectionScreen> createState() => _SavingsCollectionScreenState();
}

class _SavingsCollectionScreenState extends ConsumerState<SavingsCollectionScreen> {
  ...
  void _refresh() {
    ...
    ref.invalidate(savingsSheetProvider(key));
  }
  ...
  void _updateLocalSavingsSheetCache(int accountId, double amount) {
    try {
      final key = '${_frequency}_$_date';
      final box = Hive.box(HiveBoxes.savingsAccounts);
      final raw = box.get('savings_sheet_$key');
      if (raw != null) {
        ...
        box.put('savings_sheet_$key', data);
      }
    } catch (e) {
      debugPrint('Error updating local savings cache: $e');
    }
  }

  Future<void> _submitDeposit(int accountId, double expected) async {
    ...
    if (amount <= 0) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(isBn ? 'সঠিক জমার পরিমাণ লিখুন' : 'Please enter a valid amount'),
    ...
    try {
      ...
      try {
        ...
        if (isNetwork) {
          ...
          await syncNotifier.addToPending(
            ...
            data: payload,
            title: isBn ? 'সঞ্চয় জমা (হিসাব #$accountId: ৳$amount)' : 'Savings Deposit (Acc #$accountId: ৳$amount)',
          );
          isSavedOffline = true;
        } else {
          rethrow;
      ...
      _updateLocalSavingsSheetCache(accountId, amount);
                ...
                Expanded(
                  child: Text(
                    isSavedOffline
                        ...
                        : (isBn ? 'সঞ্চয় জমা সফল: ${CurrencyFormatter.simple(amount)}' : 'Deposit recorded: ${CurrencyFormatter.simple(amount)}'),
  ...
  Widget build(BuildContext context) {
    ...
    final sheetAsync = ref.watch(savingsSheetProvider(cacheKey));
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(
          isBn ? '$freqTitle সঞ্চয় কালেকশন' : '$freqTitle Savings Collection',
          ...
          Expanded(
            child: sheetAsync.when(
              loading: () => AppLoadingIndicator(
                message: isBn ? 'সঞ্চয় কালেকশন শিট লোড হচ্ছে...' : 'Loading savings collection list...',
              ),
  ...
  Widget _buildBody(Map<String, dynamic> data, bool isBn) {
    ...
    if (rows.isEmpty) {
      return EmptyState(
        message: isBn ? 'নির্বাচিত তারিখে কোনো সঞ্চয় কালেকশন নির্ধারিত নেই' : 'No savings scheduled for collection on this date',
      );
    }
    ...
    final filteredRows = rows.where((row) {
      ...
      if (_searchQuery.isNotEmpty) {
        ...
        return name.contains(query) || accNo.contains(query) || phone.contains(query) || memberNo.contains(query);
      }
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          ...
          _buildSearchAndFilters(rows.length, dueCount, paidCount, isBn),
          const SizedBox(height: 12),
            ...
            Container(
              ...
              child: Column(
                children: [
                  ...
                  Text(
                    isBn ? 'ফিল্টারের সাথে কোনো সদস্য মেলেনি' : 'No members match your filter',
    ...
    return Container(
      ...
      child: Column(
        children: [
          Row(
            children: [
              _buildStatItem(isBn ? 'লক্ষ্যমাত্রা' : 'Target', CurrencyFormatter.simple(expected), const Color(0xFF0F172A), Icons.assignment_outlined),
              Container(width: 1, height: 36, color: Colors.grey.shade200),
              _buildStatItem(isBn ? 'আদায় হয়েছে' : 'Collected', CurrencyFormatter.simple(collected), const Color(0xFF16A34A), Icons.check_circle_outline),
              Container(width: 1, height: 36, color: Colors.grey.shade200),
              _buildStatItem(isBn ? 'বকেয়া' : 'Due Today', CurrencyFormatter.simple(due), const Color(0xFFEA580C), Icons.pending_actions_outlined),
            ],
          ),
          const SizedBox(height: 12),
          Row(
            children: [
              ...
              Text(
                isBn
                    ...
                    : '$paidCount/$totalCount Paid (${(percent * 100).toStringAsFixed(0)}%)',
  ...
  Widget _buildSearchAndFilters(int total, int due, int paid, bool isBn) {
    return Column(
      children: [
        Container(
          height: 40,
          ...
          child: TextField(
            ...
            decoration: InputDecoration(
              hintText: isBn ? 'সদস্যের নাম, হিসাব নং বা ফোন দিয়ে খুঁজুন...' : 'Search member, account or phone...',
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              CircleAvatar(
                ...
                child: Text(
                  memberName.isNotEmpty ? memberName[0].toUpperCase() : 'S',
                  style: TextStyle(
          ...
          Container(
            ...
            child: Row(
              children: [
                _buildCardStat(isBn ? 'কিস্তির পরিমাণ' : 'Installment', CurrencyFormatter.simple(expected), const Color(0xFF334155)),
                _buildCardStat(isBn ? 'আদায় হয়েছে' : 'Collected', CurrencyFormatter.simple(collected), const Color(0xFF16A34A)),
                _buildCardStat(isBn ? 'মোট সঞ্চয়' : 'Balance', CurrencyFormatter.simple(balance), const Color(0xFF0284C7)),
              ],
            ),
          ),
          const SizedBox(height: 10),
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  InkWell(
                    onTap: () {
                      setState(() => _forceShowInputIds.add(accountId));
                    },
                    borderRadius: BorderRadius.circular(4),
                    child: Padding(
              ...
              children: [
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    onPressed: isSubmittingThis ? null : () => _submitDeposit(accountId, expected),
                    ...
                    child: isSubmittingThis
                        ...
                        : Row(
                            ...
                            children: [
                              ...
                              Text(isBn ? 'জমা নিন' : 'Collect', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13)),
                            ],
                          ),
                  ),
                ),

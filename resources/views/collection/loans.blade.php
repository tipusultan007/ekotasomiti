...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h4 class="mb-0">{{ __(ucfirst($frequency)) }} {{ __('Loan Collection') }}</h4>
    <div>
        <a href="{{ route('collection.loans.print', $frequency) }}?date={{ $date }}&area_id={{ request('area_id') }}&loan_id={{ request('loan_id') }}" class="btn btn-outline-secondary btn-sm"><i class="bi bi-printer me-1"></i>{{ __('Print') }}</a>
    </div>
</div>
...
<ul class="nav nav-pills mb-3">
    @foreach (['daily', 'weekly', 'monthly'] as $f)
        <li class="nav-item">
            <a class="nav-link {{ $f === $frequency ? 'active' : '' }}" href="{{ route('collection.loans.sheet', $f) }}">
...
<div class="row g-3 mb-4">
    ...
    <div class="col-lg-6">
        <div id="loan-details-panel" class="h-100">
            <div class="card shadow-sm border-0 mb-4 h-100" style="border-radius: 16px; border: 1px dashed #cbd5e1 !important; background: #fafafa; min-height: 380px;">
                <div class="card-body text-center text-muted py-5 d-flex flex-column align-items-center justify-content-center">
                    ...
                    <h6 class="fw-bold text-dark mb-1">{{ __('No Loan Selected') }}</h6>
                    <p class="small text-muted mb-0" style="max-width: 250px;">{{ __('Choose a member loan from the left form to view live dues and repayment schedule.') }}</p>
                </div>
            </div>
        </div>
    </div>
...
<form class="row g-2 mb-3" method="GET">
    ...
    <div class="col-md-3">
        <select name="loan_id" class="form-select select2" data-placeholder="{{ __('All :freq loans', ['freq' => __(ucfirst($frequency))]) }}">
            <option value="">{{ __('All :freq loans', ['freq' => __(ucfirst($frequency))]) }}</option>
            @foreach ($loanOptions as $loan)
                <option value="{{ $loan->id }}" @selected(request('loan_id') == $loan->id)>
...
<div class="row g-3 mb-3">
    <div class="col-6 col-md">
        <div class="card shadow-sm"><div class="card-body">
            <div class="text-muted small">{{ $frequency === 'monthly' ? __('Due This Month') : __('Due Today') }}</div>
            <div class="fs-5 fw-bold">৳{{ number_format($totals['due_today'], 2) }}</div>
        </div></div>
    </div>
    <div class="col-6 col-md">
        <div class="card shadow-sm"><div class="card-body">
            <div class="text-muted small">{{ __('Overdue') }}</div>
            <div class="fs-5 fw-bold {{ $totals['overdue'] > 0 ? 'text-danger' : '' }}">৳{{ number_format($totals['overdue'], 2) }}</div>
        </div></div>
    </div>
    <div class="col-6 col-md">
        <div class="card shadow-sm"><div class="card-body">
    ...
    <div class="col-6 col-md">
        <div class="card shadow-sm"><div class="card-body">
            ...
            <div class="fs-5 fw-bold text-danger">৳{{ number_format($totals['total_due'], 2) }}</div>
        </div></div>
    </div>
    <div class="col-6 col-md">
        <div class="card shadow-sm"><div class="card-body">
            <div class="text-muted small">{{ __('Loans') }}</div>
            <div class="fs-5 fw-bold">{{ $rows->count() }}</div>
        </div></div>
    </div>
</div>

<div class="card shadow-sm">
    <div class="card-header bg-white"><span class="fw-semibold">{{ __('Loan Collection List') }}</span></div>
            ...
            <tbody>
                @forelse ($transactions as $txn)
                    <tr>
                        ...
                        <td class="fw-semibold">{{ $txn->loan->loan_no }}</td>
                        <td>{{ $txn->loan->member->name }}</td>
                        ...
                        @if(auth()->user()->hasAnyRole(['admin', 'super_admin', 'manager']) || auth()->user()->can('reverse transactions'))
                            <td class="text-end">
                                ...
                                <form method="POST" action="{{ route('collection.loans.transactions.destroy', $txn) }}" class="d-inline" data-confirm="{{ __('Delete repayment :no? The repayment will be permanently removed, the loan schedule restored, and the linked cash entry removed.', ['no' => $txn->txn_no]) }}">
                                    @csrf
                ...
                @empty
                    <tr><td colspan="{{ (auth()->user()->hasAnyRole(['admin', 'super_admin', 'manager']) || auth()->user()->can('reverse transactions')) ? 7 : 6 }}" class="text-center text-muted py-4">{{ __('No repayments recorded for this date.') }}</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
        ...
        <form method="POST" id="editLoanTxnForm">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">{{ __('Edit Loan Repayment') }}</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <div class="mb-3">
                        <label class="form-label">{{ __('Transaction No') }}</label>
                        <input type="text" class="form-control" id="editLoanTxnNo" disabled>
                    </div>
                    <div class="row g-3">
                        <div class="col-6">
                            <label class="form-label">{{ __('Amount') }}</label>
                            <input type="number" name="amount" step="0.01" min="0.01" class="form-control" id="editLoanTxnAmount" required>
                        </div>
                        <div class="col-6">
                    ...
                    <div class="mb-3 mt-3">
                        ...
                        <textarea name="notes" class="form-control" id="editLoanTxnNotes" rows="2"></textarea>
                    </div>
                </div>
...
<script>
    (function () {
        ...
        const url = @json(route('collection.loans.details', $frequency));
        const date = @json($date);
        const placeholder = '<div class="card shadow-sm border-0 mb-4 h-100" style="border-radius: 16px; border: 1px dashed #cbd5e1 !important; background: #fafafa; min-height: 380px;"><div class="card-body text-center text-muted py-5 d-flex flex-column align-items-center justify-content-center"><div class="p-3 rounded-circle bg-white shadow-xs mb-3 border"><i class="bi bi-cash-stack fs-2 text-primary"></i></div><h6 class="fw-bold text-dark mb-1">' + @json(__('No Loan Selected')) + '</h6><p class="small text-muted mb-0" style="max-width: 250px;">' + @json(__('Choose a member loan from the left form to view live dues and repayment schedule.')) + '</p></div></div>';

        if (!select || !panel) return;

        function load(id) {
            if (!id) { panel.innerHTML = placeholder; return; }
            fetch(url + '?loan_id=' + encodeURIComponent(id) + '&date=' + encodeURIComponent(date))
                .then(r => r.text())
